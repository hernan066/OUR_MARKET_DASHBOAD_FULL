...
function ExpensesEdit({ expenses }) {
  const navigate = useNavigate();
  const { id } = useParams();
  const [createExpenses, { isLoading, isError }] = usePutExpensesMutation();
  ...
  const formik = useFormik({
    initialValues: {
      ...
      date: dayjs(expenses.date),
    },
    onSubmit: async (values) => {
      const res = await createExpenses({ id, ...values }).unwrap();
      if (res) {
        Swal.fire({
          position: "center",
          icon: "success",
      ...
      >
        ...
        >
          ...
          <TextField
            ...
            label="Gasto"
            name="expensesName"
            value={formik.values.expensesName}
          ...
          {isError && <Alert severity="error">Error — Gasto no editado</Alert>}
        </Box>
      </Box>
    </MDBox>
  );
